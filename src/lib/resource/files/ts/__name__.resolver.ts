...
export class <%= classify(name) %>Resolver {
  constructor(private readonly <%= lowercased(name) %>Service: <%= classify(name) %>Service) {}<% if (crud && type === 'graphql-code-first' && isMongoose) { %>
  ...
  @Mutation(() => <%= singular(classify(name)) %>)
  remove<%= singular(classify(name)) %>(@Args('id', { type: () => ID }) id: string) {
  ...
  findOne(@Args('id') id: <% if ((isMongoose || (isTypeOrm && db === 'mongodb'))) { %>string<% } else { %>number<% } %>) {
    return this.<%= lowercased(name) %>Service.findOne(id);
  }
  ...
  remove(@Args('id') id: <% if ((isMongoose || (isTypeOrm && db === 'mongodb'))) { %>string<% } else { %>number<% } %>) {
    return this.<%= lowercased(name) %>Service.remove(id);
  }<% } %>
}
