...
export class <%= classify(name) %>Gateway {
  ...
  findOne(@MessageBody() id: <% if ((isMongoose || (isTypeOrm && db === 'mongodb'))) { %>string<% } else { %>number<% } %>) {
  ...
  remove(@MessageBody() id: <% if ((isMongoose || (isTypeOrm && db === 'mongodb'))) { %>string<% } else { %>number<% } %>) {
